...
public:
	...
	enum PrimitiveType {PLANE, SPHERE, SQUARE};
	class Primitive
	{
	public:
	...
	{
	public:
		Square() : Primitive(SQUARE), m_normal(glm::vec3(0.0f, 1.0f, 0.0f)), m_value(0.0f) {init_visualization();};
		Square(const glm::vec3 norm, float value) : Primitive(SQUARE), m_normal(norm), m_value(value) {init_visualization();};
		Square(const Square& other) : Primitive(other), m_normal(other.m_normal), m_value(other.m_value) {};
		virtual ~Square() {};
...
public:
	...
	bool intersection();
	//PrimitiveType get_primitive(int primitiveNum);
	int get_size();
	void update();
