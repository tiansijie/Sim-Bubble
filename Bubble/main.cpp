...
void aimCamera(void)
{
	...
	glm::mat4 projection = glm::perspective(70.0f, static_cast<float>(window_width) / static_cast<float>(window_height), 0.5f, 600.0f);
...
int main(int argc, char** argv)
{
	...
	//for(int i = 0; i< 30;i++){
		//srand (time(NULL)); 
	//	myBubble.insertBubble(glm::vec3(rand() % 12, 5.0f + rand() % 12, rand() % 12), glm::vec3(0,0,0), rand()%4);
	//}

	//myBubble.insertBubble(center, center, 3.0f);
	//myBubble.insertBubble(center2, center2, 3.0f);

	scene.insert_primitive(new Scene::Plane(normalPlane, valuePlane));

	/*normalPlane = glm::vec3(1,0,0);
	scene.insert_primitive(new Scene::Plane(normalPlane, valuePlane));*/

	normalPlane = glm::vec3(1,0,0);
	//scene.insert_primitive(new Scene::Square(normalPlane, 20));
	//myBubble.addSoild(normalPlane, normalPlane * 20.f);	
	...
	while(run)
	{
		if(timeBubble == 20 && myBubble.size() < 50){
			myBubble.insertBubble(glm::vec3(rand() % 20, 5 + rand() % 15, rand() % 20), glm::vec3(0,0,0), 4 + rand() % 4);
			//myBubble.insertBubble(glm::vec3(0.0f + posT, 0.7f, 0.0f + posT), glm::vec3(0,0,0), 4);
			posT += 4;
		}
		/*5.0f + rand() % 12*/
		timeBubble ++;
		...
		myBubble.update(0.008f);		
//		myBubble.draw(vbo_handle);
		deactivate_shaderprog(m_shaderprog_handle);
		
		//scene.update();//Sijie Update position of bubble
		drawAxes();
		if(!pause && record)
			grabScreen();
		frame_num++;
